...
dt.merge.cor <- merge(dt.cluster.miRTarGene.Neg.cor, dt.gene.name,
                      by.x = 'ID.gene', by.y = 'ENSEMBL', all.x = T)

dt.merge.cor.naomit <- dt.merge.cor[SYMBOL!='']
...
dt.gene.in.tier1pathway <- sapply(order, function(x){
                              ...
                              lst.TaGene.pathway <- intersect(dt.merge.cor.naomit$ID.gene,lst.gene.pathway )
...
dt.gene.pathway.sympol <- merge(dt.gene.pathway, dt.gene.name, 
                                by.x = 'ID.gene', by.y = 'ENSEMBL', all.x = T)
dt.gene.pathway.sympol.naomit <- dt.gene.pathway.sympol[SYMBOL!='']
...
dt.miRNA.gene <- data.table(source = dt.merge.cor.naomit$ID.miRNA,
                            target = dt.merge.cor.naomit$SYMBOL,
                            value =  dt.merge.cor.naomit$cor)

dt.pathway.gene <- data.table(source = dt.gene.pathway.sympol.naomit$pathway,
                              target =dt.gene.pathway.sympol.naomit$SYMBOL,
                              value = 1)
dt.network <- rbind(dt.miRNA.gene, dt.pathway.gene)

write.csv(dt.network, './tables/dt.network.sympol.csv', row.names = F, quote = F)
saveRDS(dt.pathway.gene, './RDS/Pathway_result/dt.pathway.gene.sympol.rds')
...
# try to get miRNA target genes included in network1 ----------------------

dt.network1.gene <- dt.pathway.gene[1:44]
lst.gene.in.network1 <-  dt.network1.gene$target%>%unique()
dt.miRNA.network1 <- dt.miRNA.gene[target%in%lst.gene.in.network1]%>%unique()

table(dt.miRNA.network1$source)


dt.network2.gene <- dt.pathway.gene[45:nrow(dt.pathway.gene)]
lst.gene.in.network2 <-  dt.network2.gene$target%>%unique()
dt.miRNA.network2 <- dt.miRNA.gene[target%in%lst.gene.in.network2]%>%unique()

table(dt.miRNA.network2$source)
