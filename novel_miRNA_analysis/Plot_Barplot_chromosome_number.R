...
dt.miRNA.total.annotation <- readRDS('./RDS/miRNA_chromosome_annotation_total.rds') 
...
ggplot(dt.N.miRNA, aes(x = chromosome.name, y = N.miRNA, group = type.miRNA))+
  ...
  labs(x= 'Chromosome', y = 'Number of miRNA')+
  theme_bw()+
  scale_fill_manual(values = c('#a2d5f2','#07689f'))+
  theme(axis.text.x = element_text(size=16,face='bold',color='gray40', angle = 45, vjust = 0.5),
        axis.text.y = element_text(size = 16, face = 'bold'),
        axis.title = element_text(size=16,face='bold'),
        legend.title = element_text(size=16,face='bold'),
        legend.text = element_text(size=16,face='bold'))
ggsave('./charts/Fig_7a_Barplot_number_miRNA_chromosome.pdf', width = 14, height = 5)
